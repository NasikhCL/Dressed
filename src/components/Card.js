...
export default function Card({ userData }) {
  ...
  useEffect(() => {
    ...
    const isCardFav = localStorageFavCards.find(
      (card) => card.id === userData.id
    );
    if (isCardFav) {
      setIsFav(true);
    }
  ...
  useEffect(() => {
    ...
    if (isFav) {
      ...
      const updatedFavCards = localStorageFavCards.filter(
        (card) => card.id !== userData.id
      );
  ...
  return (
    <div className="border rounded-lg shadow-md max-w-sm lg:w-[340px] w-[300px] mt-7 mx-1 ">
      <div className="h-96">
        <img
        ...
        <div className="flex justify-between">
          ...
          <button onClick={handleFavClick}>
            {isFav ? (
              ...
              <h4 className="text-gray-400 text-2xl cursor-pointer">
